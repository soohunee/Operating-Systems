...
if options.solve == False:
    ...
    print(policy)
    print(cachesize)
    print('%d\n%d\n%.2f' % (hits, miss, (100.0*float(hits))/(float(hits)+float(miss))))
    print('')
'''
	file.write(str(policy))
	file.write(str(cache.size))
	hitrate = 100.0*float(hits)/float(hits)
	file.write(str(hits), str(miss), str(hitrate))
	file.close()
	'''
